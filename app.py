...
st.set_page_config(page_title="NIFTY Weekly 300-Candle Terminal", layout="wide")
...
def calculate_macd_and_signal(df):
    # Updated to 300 samples as requested
    ...
    # NON-REPAINT: Checking closed candle at -2
...
def process_data(df_raw):
    ...
    # Keeping 15min timeframe as per previous architecture
...
if session_token:
    try:
        ...
        expiry_iso = get_next_weekly_expiry()

        with st.spinner("Analyzing 300 Candles (Weekly Expiry)..."):
            # Increased timedelta to 20 days to ensure 300 15-min candles are available
            to_d = datetime.now()
            ...
            def get_best_strike(right):
                ...
                # Price target changed from 60 to 50
            ...
            for key, stat, label in [("idx", stat_idx, "INDEX"), ("ce", stat_ce, f"CALL {c_s}"), ("pe", stat_pe, f"PUT {p_s}")]:
                if stat in ["BUY", "SELL"] and stat != st.session_state.last_signals[key]:
                    send_telegram(f"📢 <b>{label}: {stat}</b>\nWeekly Expiry: {expiry_iso[:10]}\nConfirmed at {t_now}")
                    st.session_state.last_signals[key] = stat
                    alert_triggered = True
            
            if alert_triggered: play_sound()

            # --- UI ---
            st.title("🏛 NIFTY Weekly Terminal (300 Samples)")
            st.info(f"Scanning Weekly Expiry: {expiry_iso[:10]} | Target Premium: ₹50")
            
            c1, c2, c3 = st.columns(3)
            show_indicator(c1, "NIFTY INDEX", "SPOT", df_idx['close'].iloc[-1], stat_idx)
            show_indicator(c2, f"CALL {c_s}", "WEEKLY", c_ltp, stat_ce)
            show_indicator(c3, f"PUT {p_s}", "WEEKLY", p_ltp, stat_pe)
            ...
            plot_with_arrows(ax0, m_idx, s_idx, h_idx, arr_idx, "INDEX MACD (300 Samples)")
            plot_with_arrows(ax1, m_ce, s_ce, h_ce, arr_ce, f"WEEKLY CALL {c_s} MACD")
            plot_with_arrows(ax2, m_pe, s_pe, h_pe, arr_pe, f"WEEKLY PUT {p_s} MACD")
            
            plt.xlabel("Candle Count (Last 300)")
            st.pyplot(fig)
