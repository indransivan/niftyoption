...
# Auto-refresh every 5 minutes (300,000ms) to sync with 15min candles
...
def show_indicator(col, title, status, ltp):
    ...
    col.markdown(f"""
        <div style="background-color:{bg}; padding:20px; border-radius:12px; text-align:center; border: 1px solid #444; margin-bottom:10px;">
            <p style="margin:0; color: white; font-size: 1rem; opacity: 0.8;">{title}</p>
            <h1 style="margin:10px 0; color: white; font-size: 2.2rem; letter-spacing: 2px;">{status}</h1>
...
def draw_combined_chart(df, macd_line, signal_line, hist, macd_signals, title):
    ...
    fig.add_trace(go.Scatter(x=x_vals, y=macd_line, line=dict(color='#3498db'), name='MACD'), row=2, col=1)
    fig.add_trace(go.Scatter(x=x_vals, y=signal_line, line=dict(color='orange', dash='dot'), name='Signal'), row=2, col=1)
    
    h_colors = ['#26a69a' if val > 0 else '#ef5350' for val in hist]
    fig.add_trace(go.Bar(x=x_vals, y=hist, marker_color=h_colors, name='Hist'), row=2, col=1)
    ...
    if buy_x:
        fig.add_trace(go.Scatter(x=buy_x, y=buy_y, mode='markers', marker=dict(color='#00ff88', size=10, symbol='triangle-up'), name='BUY'), row=2, col=1)
    if sell_x:
        fig.add_trace(go.Scatter(x=sell_x, y=sell_y, mode='markers', marker=dict(color='#ff4444', size=10, symbol='triangle-down'), name='SELL'), row=2, col=1)

    fig.update_layout(height=500, template="plotly_dark", xaxis_rangeslider_visible=False, title=title)
...
if session_token:
    try:
        ...
        def fetch_data(stock_code, exchange_code, product_type, strike="", right=""):
            res = breeze.get_historical_data(
                ...
                expiry_date=expiry_iso if product_type=="options" else "", 
                right=right, 
                strike_price=strike
            )
            df = process_data(pd.DataFrame(res["Success"]))
            m_line, s_line, hist = calculate_macd(df)
            dirs, stat, sigs = calculate_macd_signals(m_line)
            
            if stat in ["BUY", "SELL"]:
                asset_name = f"{stock_code} {strike} {right.upper()}".strip()
                msg = f"🚀 {stat} SIGNAL: {asset_name}\nLTP: ₹{df['close'].iloc[-1]}"
                send_telegram(msg)
                
            return df, m_line, s_line, hist, stat, sigs

        # Fetch Index, Call, and Put
        df_idx, m_idx, sl_idx, h_idx, stat_idx, sig_idx = fetch_data("NIFTY", "NSE", "cash")
        df_ce, m_ce, sl_ce, h_ce, stat_ce, sig_ce = fetch_data("NIFTY", "NFO", "options", c_s, "call")
        df_pe, m_pe, sl_pe, h_pe, stat_pe, sig_pe = fetch_data("NIFTY", "NFO", "options", p_s, "put")
        ...
        show_indicator(idx_cols[0], "NIFTY INDEX", stat_idx, df_idx['close'].iloc[-1])
        show_indicator(idx_cols[1], f"CALL {c_s}", stat_ce, df_ce['close'].iloc[-1])
        show_indicator(idx_cols[2], f"PUT {p_s}", stat_pe, df_pe['close'].iloc[-1])

        # Charts
        draw_combined_chart(df_idx, m_idx, sl_idx, h_idx, sig_idx, "NIFTY SPOT INDEX (15m)")
        draw_combined_chart(df_ce, m_ce, sl_ce, h_ce, sig_ce, f"NIFTY CALL {c_s} (15m)")
        draw_combined_chart(df_pe, m_pe, sl_pe, h_pe, sig_pe, f"NIFTY PUT {p_s} (15m)")
